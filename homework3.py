...
@task(name="create_X_linear_regression")
def create_X_linear_regression(df, dv=None):
    categorical = ['PULocationID', 'DOLocationID']
    numerical = ['trip_distance']
    dicts = df[categorical + numerical].to_dict(orient='records')
...
def train_linear_regression(X_train, y_train, X_val, y_val, dv):    
    with mlflow.start_run() as run:
        ...
        return model, dv, run.info.run_id

@flow(name="run_duration_prediction")
def run(year, month):
    ...
    X_train, dv = create_X_linear_regression(df_train)
    X_val, _ = create_X_linear_regression(df_val, dv)
    ...
    model, dv, run_id = train_linear_regression(X_train, y_train, X_val, y_val, dv)
